...
echo     if (!item) return res.status(404).json({ error: 'Item not found' }); >> "%ROUTE_FILE%"
echo     res.json(item);>> "%ROUTE_FILE%"
echo     } catch (err) {>> "%ROUTE_FILE%"
echo      console.error('Error fetching item by ID:', err);>> "%ROUTE_FILE%"
echo      res.status(500).json({ error: 'Server error while fetching item' }); >> "%ROUTE_FILE%"
echo  }>> "%ROUTE_FILE%"
echo });>> "%ROUTE_FILE%"
