...
echo async function connect({  >>"%DB_FILE%"
echo host='%DB_URI%',  >>"%DB_FILE%"
echo dbName='%DB_NAME%',  >>"%DB_FILE%"
echo  USER = '%USERNAME%',  >>"%DB_FILE%"
echo  PASS = '%PASSWORD%'  >>"%DB_FILE%"
echo }) { >>"%DB_FILE%" 

echo // Build auth string if both USER and PASS are provided >>"%DB_FILE%" 
echo const auth = USER ^&^& PASS ^? `^${encodeURIComponent^(USER^)}:^${encodeURIComponent^(PASS^)}^@` : ''; >>"%DB_FILE%"

echo // Construct URI for MongoDB Atlas  >>"%DB_FILE%"
echo const uri = `mongodb^+srv:^/^/^${auth}^${host}^/^${dbName}^?retryWrites=true^&w=majority`; >>"%DB_FILE%"
echo console.log(`Connecting to MongoDB Atlas at ^${uri}`); >>"%DB_FILE%"
...
echo     useNewUrlParser: true, >>"%DB_FILE%"
echo     useUnifiedTopology: true>>"%DB_FILE%"
echo     }); >>"%DB_FILE%"

echo     console.log('Connection established to MongoDB');>>"%DB_FILE%"
echo     return client.db(dbName);>>"%DB_FILE%"
echo   } catch (err) {>>"%DB_FILE%"
echo     console.error('Connection failed:', err.message);>>"%DB_FILE%"
...
echo // Define your MongoDB Atlas credentials and DB name once >>"%SERVICE_FILE%"
echo const connectionParams = { >>"%SERVICE_FILE%"
echo   host: '%DB_URI%', >>"%SERVICE_FILE%"
echo   dbName: '%DB_NAME%', >>"%SERVICE_FILE%"
echo   USER: '%USERNAME%', >>"%SERVICE_FILE%"
echo   PASS: '%PASSWORD%' >>"%SERVICE_FILE%"
echo }; >>"%SERVICE_FILE%"


echo async function fetchData() {>>"%SERVICE_FILE%"
echo   const db = await connect(connectionParams); >>"%SERVICE_FILE%"
...
echo   return (await (await connect(connectionParams))>>"%SERVICE_FILE%"
